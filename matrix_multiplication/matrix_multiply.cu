...
/* Kernel for matrix multiplication, w/o parallelization -> every thread works on all matrix elements */
...
int main(int argc, char * argv[])
{
  ...
  if ( argc != 2 ) {
    cout << "Need two arguments: number of columns (= number of rows) of matrix" << endl;
    return -1;
  }
