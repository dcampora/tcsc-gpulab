 ...
 *
 *          A grid and block dimension-strided version is included as an
 * optional commented out version. This version would work irrespective of the
 * number of blocks and threads it is invoked with.
 */
__global__ void multiply_square_matrices(const int size, const float *A,
  ...
  // Optional solution: Grid and block dimension-strided for-loop which works
  // for any grid and block dimension for (int i = blockIdx.x * blockDim.x +
  // threadIdx.x; i < size; i += gridDim.x * blockDim.x) {
  //   for (int j = blockIdx.y * blockDim.y + threadIdx.y; j < size; j +=
