  ...
  const Track& track)
{
  ...
  */
  ...
  const int first_x = hits[first_hit].x;
  const int first_y = hits[first_hit].y;
  const int radius_first = std::sqrt(first_x*first_x + first_y*first_y);
  const int last_x = hits[last_hit].x;
  const int last_y = hits[last_hit].y;
  const int radius_last = std::sqrt(last_x*last_x + last_y*last_y);
  ...
  const int event_offset)
{
  ...
  */
  // TODO:
  const my_float_t first_x = 1.f;      // hits[first_hit].x;
  const my_float_t first_y = 1.f;      // hits[first_hit].y;
  const my_float_t radius_first = 1.f; // std::sqrt(first_x*first_x + first_y*first_y);
  const my_float_t last_x = 1.f;       // hits[last_hit].x;
  const my_float_t last_y = 1.f;       // hits[last_hit].y;
  const my_float_t radius_last = 1.f;  // std::sqrt(last_x*last_x + last_y*last_y);
  ...
  State state;
  // TODO:
  state.x = 1.f; // hits[ track.hits[first_hit_index] ].x;
  state.y = 1.f; // hits[ track.hits[first_hit_index] ].y;
  state.z = 1.f; // hits[ track.hits[first_hit_index] ].z;
  state.tx = stateAtBeamLine.tx;
  state.ty = stateAtBeamLine.ty;
  ...
  for (uint i = first_hit_index + dhit; i != last_hit_index + dhit; i += dhit) {
    int hit_index = track.hits[i];
    // TODO:
    const auto hit_x = 1.f; // hits[hit_index].x;
    const auto hit_y = 1.f; // hits[hit_index].y;
    const auto hit_z = 1.f; // hits[hit_index].z;
...
void kalman_filter_gpu(
  ...
  for ( int track_number = threadIdx.x; track_number < number_of_tracks; track_number += blockDim.x ) {
    ...
    State state = simplified_fit(dev_hits_soa, state_at_beamline, track, event_offsets_hits[event_number]);
