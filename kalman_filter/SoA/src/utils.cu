...
void get_hits(Hit *host_hits,
  ...
  for ( int event_number = 0; event_number < max_events; ++event_number ) {
    ...
    for ( uint32_t i = 0; i < n_hits; ++i ) {
      ...
      host_hits_soa.x[accumulated_size + accumulated_hit_number] = hit.x;
      host_hits_soa.y[accumulated_size + accumulated_hit_number] = hit.y;
      host_hits_soa.z[accumulated_size + accumulated_hit_number] = hit.z;
      host_hits_soa.mcp[accumulated_size + accumulated_hit_number] = hit.mcp;
