                   ...
                   int const n_streams)
{
  ...
  for ( int repetition = 0; repetition < n_repetitions; ++repetition) {
    ...
    /* to do: copy the remaining necessary host arrays to the device, following the example of the hits*/
    for (int i_stream = 0; i_stream < n_streams; i_stream++) {
    ...
    /* to do: Launch kernel for every stream */

    /* to do: Copy back states to host for every steram*/

    /* to do: synchronize to make sure a stream has finished
     use the stream specific function for synchronization this time */
