                   ...
                   int const)
{
  ...
  Track *host_tracks;
  State *states_gpu;
  CUDA_ASSERT( cudaMallocHost( (void**) &host_hits, max_hits_per_event * max_events * sizeof(Hit) ) ); // for all hits in all events
  CUDA_ASSERT( cudaMallocHost( (void**) &host_tracks, max_tracks_per_event * max_events * sizeof(Track) ) ); // for all tracks in all events
  CUDA_ASSERT( cudaMallocHost( (void**) &states_gpu, max_tracks_per_event * max_events * sizeof(State) ) ); // for all states in all events
  ...
  for ( int repitition = 0; repitition < n_repetitions; ++repitition) {
    ...
    dim3 threads(32);
    kalman_filter_gpu<<<blocks, threads>>>(dev_hits, dev_tracks, dev_event_offsets_hits, dev_event_offsets_tracks, dev_states, max_events);
  ...
  /* Free host arrays */
  //delete [] host_hits;
  CUDA_ASSERT( cudaFreeHost( host_hits ) );
  CUDA_ASSERT( cudaFreeHost( host_tracks ) );
  CUDA_ASSERT( cudaFreeHost( states_gpu ) );
  delete [] states_cpu;
